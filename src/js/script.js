$(document).ready(function() {
  $('.footer').on('mouseenter', function() {
    ...
    //$('.enter-aside').css('display', 'flex');
  });
});

function checkMediaQuery() {
  if (window.matchMedia("(max-width: 1825px)").matches) {
    let scroll_pos = 0;
...
/*function checkMediaQuery() {
  const large = window.matchMedia('(max-width: 1825px)')
  const middle = window.matchMedia('(max-width: 1400px)')
  const small = window.matchMedia('(max-width: 575px)')
  large.addEventListener(function(e) {
